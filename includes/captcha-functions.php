...
function rcp_validate_captcha() {
	...
	if (isset($rcp_options['enable_recaptcha']) && !empty($rcp_options['recaptcha_public_key'])) {
		...
		$resp = rcp_recaptcha_check_answer($privatekey, rcp_get_ip(), $_POST['secret'], $_POST['response']);
...
function rcp_recaptcha_check_answer($secret, $remoteip, $challenge, $response, $extra_params = array()) {
	if ($secret == null || $secret == '') {
		die("To use reCAPTCHA you must get an API key from <a href='https://www.google.com/recaptcha/admin/create'>https://www.google.com/recaptcha/admin/create</a>");
	}
	
	$response = rcp_recaptcha_http_post(RECAPTCHA_VERIFY_SERVER, "/recaptcha/api/siteverify", array('secret' => $secret, 'remoteip' => $remoteip, 'response' => $response) + $extra_params);
 ...
 */
function rcp_recaptcha_http_post($host, $path, $data, $port = 80) {
	
	$req = rcp_recaptcha_qsencode($data);
 ...
 */
function rcp_recaptcha_qsencode($data) {
	$req = "";
